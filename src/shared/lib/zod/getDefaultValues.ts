...
export function getDefaultValues<T extends z.ZodType>(schema: T): z.output<T> {
	const def = (schema._def || (schema as z.ZodType & { def?: ZodDefWithShape }).def) as ZodDefWithShape
	...
	switch (def.typeName || def.type) {
		case 'object': {
			...
			for (const [key, value] of Object.entries(shape as Record<string, z.ZodType>)) {
				if (value) outputObject[key] = getDefaultValues(value as z.ZodType)
			}
		...
		case 'number':
			...
			for (const check of def.checks || []) {
				if (check._zod?.def && ['greater_than', 'less_than'].includes(check._zod?.def?.check ?? '')) {
		...
		case 'pipe': {
			...
			const pipeInput = def.in ? getDefaultValues(def.in) : undefined
		...
		case 'array':
			return [] as z.output<T>
		case 'tuple':
			return (def.items || []).map((item: z.ZodType) => getDefaultValues(item)) as z.output<T>
		case 'set':
			...
			if (def.entries) {
				...
				if (hasNumericValues) {
					...
					return values.filter(
						value => typeof (entries as Record<string, unknown>)[value as string | number] !== 'number'
					)[0] as z.output<T>
				}
		...
		case 'union': {
			...
			return (options?.[0] ? getDefaultValues(options[0]) : undefined) as z.output<T>
		}
		case 'intersection': {
			const left = def.left ? getDefaultValues(def.left) : {}
			const right = def.right ? getDefaultValues(def.right) : {}
		...
		case 'lazy': {
			...
			return (lazyType ? getDefaultValues(lazyType) : undefined) as z.output<T>
		}
		case 'pipeline': {
			// For pipelines, we need to actually run through the pipeline
			// to get the final transformed result
			const input = def.in ? getDefaultValues(def.in) : undefined
			try {
		...
		case 'default': {
			...
			const innerDef = defaultInnerType?._def || (defaultInnerType as z.ZodType & { def?: ZodDefWithShape })?.def
			const isFunction =
				(innerDef as ZodDefWithShape)?.typeName === 'ZodFunction' || (innerDef as ZodDefWithShape)?.type === 'function'
		...
		case 'optional': {
			...
			if (innerOptional) {
				return getDefaultValues(innerOptional) as z.output<T>
			}
			return null as z.output<T>
		}
		case 'nullable': {
			...
			if (innerTypeForNullable) {
				const innerDefForNullable =
					innerTypeForNullable._def || (innerTypeForNullable as z.ZodType & { def?: ZodDefWithShape }).def
				if (innerDefForNullable && 'coerce' in innerDefForNullable && innerDefForNullable.coerce) {
					return getDefaultValues(innerTypeForNullable) as z.output<T>
				}
			}
		...
		case 'branded': {
			...
			return (brandedType && typeof brandedType !== 'string' ? getDefaultValues(brandedType) : undefined) as z.output<T>
		}
		case 'catch':
			...
			try {
				...
				const innerValue = innerType ? getDefaultValues(innerType) : undefined
				return schema.parse(innerValue) as z.output<T>
			} catch {
				// Get the catch value
				const catchValue =
					typeof def.catchValue === 'function' ? def.catchValue({ error: new Error() }) : def.catchValue
		...
		case 'template_literal':
			...
			if (def.parts) {
				return (def.parts as (string | number | z.ZodType)[])
					.map(part => {
						...
						// For schema parts, getDefaultValues with empty string
						return ''
					})
					.join('') as z.output<T>
			}
			return '' as z.output<T>
		case 'readonly': {
			// For readonly, getDefaultValues the inner type
			const readonlyInner = def.innerType || def.base
			return (readonlyInner ? getDefaultValues(readonlyInner) : undefined) as z.output<T>
		}
		case 'custom': {
			return null as z.output<T>
		}
